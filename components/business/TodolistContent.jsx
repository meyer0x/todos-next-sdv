...
const TodoListContent = props => {
	...
	if (!todolist)
		return (
			<Text className="p-6 text-center">Please, choose or add a list.</Text>
		);

	if (todolist.todos.length === 0)
		return <Text className="p-6 text-center">Please, add a todo.</Text>;

	return (
		<div className="flex-1 overflow-y-scroll">
			{todolist.todos
				.filter(({ done }) => (isFiltered ? !done : true))
				.map(todo => (
					<div
						className="flex flex-row items-center justify-between p-6 border-t border-b border-slate-200 group"
						key={todo.label}
					>
